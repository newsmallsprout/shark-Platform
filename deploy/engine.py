...
from .models import DeployPlan, Server
...
"""
    ...
    def generate_compose(self, req: DeployRequestSchema) -> str:
        parts = []
    ...
    def generate_k8s(self, req: DeployRequestSchema) -> List[str]:
        manifests = []
        for svc in req.services:
            name = svc.name
            image = {
...
"""
    ...
    def _ssh_cmd(self, server: ServerConfigSchema, cmd: str) -> List[str]:
    ...
    def _scp_cmd(self, server: ServerConfigSchema, local: str, remote: str) -> List[str]:
        if server.auth_method == "key" and server.key_path:
...
"""
    ...
    def _install_mysql_script(self, svc: Dict) -> List[str]:
        ...
        cmds.append("apt-get install -y libaio1 libncurses5 || yum install -y libaio ncurses-libs || true")
...
"""
    ...
    def _artifact_path(self, task_id: str, filename: str) -> str:
        d = os.path.join(settings.BASE_DIR, "deploy_artifacts", task_id)
        os.makedirs(d, exist_ok=True)
        return os.path.join(d, filename)

    def run(self, req: DeployRequestSchema) -> DeployPlan:
        # Normalize versions
        for svc in req.services:
        ...
        plan_obj = DeployPlan(id=req.task_id, req=req.model_dump())
        plan_obj.save() # Initial save

        try:
            reqs = []
            tgts = []
            if req.environment == "machine":
                reqs.extend(["wget", "tar", "systemd"])
                names = [s.name for s in req.services]
                if "mysql" in names:
                    reqs.extend(["libaio", "ncurses"])
            elif req.environment == "docker":
                reqs.extend(["docker", "docker compose"])
            ...
            for sid in req.server_ids:
                try:
                    s = Server.objects.get(id=sid)
                    servers.append(s)
                    # Convert to ServerConfigSchema for helper methods
                    s_schema = ServerConfigSchema(
                        id=s.id, name=s.name, host=s.host, port=s.port,
                        user=s.user, auth_method=s.auth_method,
                        password=s.password, key_path=s.key_path
                    )
                    tgts.append(f"{s.user}@{s.host}:{s.port}")
                except Server.DoesNotExist:
                    continue
            
            plan_obj.requirements = reqs
            plan_obj.targets = tgts
            plan_obj.status = "running"
            plan_obj.save()

            if req.environment == "docker":
                compose_text = self.generate_compose(req)
                fp = self._artifact_path(req.task_id, "docker-compose.yml")
                with open(fp,"w",encoding="utf-8") as f:
                    f.write(compose_text)
                plan_obj.artifacts.append(fp)

                service_names = [s.name for s in req.services]
                if "nginx" in service_names:
                    conf_path = self._artifact_path(req.task_id, "nginx.conf")
                    with open(conf_path, "w", encoding="utf-8") as f:
                        f.write("events { worker_connections 1024; }\\nhttp { server { listen 80; location / { return 200 'OK'; } } }")
                    plan_obj.artifacts.append(conf_path)
                
                if "prometheus" in service_names:
                    prom_path = self._artifact_path(req.task_id, "prometheus.yml")
                    with open(prom_path, "w", encoding="utf-8") as f:
                        f.write("global:\\n  scrape_interval: 15s\\nscrape_configs:\\n  - job_name: 'prometheus'\\n    static_configs:\\n      - targets: ['localhost:9090']")
                    plan_obj.artifacts.append(prom_path)

                for server in servers:
                    s_schema = ServerConfigSchema(
                        id=server.id, name=server.name, host=server.host, port=server.port,
                        user=server.user, auth_method=server.auth_method,
                        password=server.password, key_path=server.key_path
                    )
                    cmds = []
                    ...
                    for art in plan_obj.artifacts:
                        remote_path = f"/root/deploy/{req.task_id}/{os.path.basename(art)}"
                        plan_obj.commands.append(" ".join(self._scp_cmd(s_schema, art, remote_path)))
                        if req.execute:
                            subprocess.run(self._scp_cmd(s_schema, art, remote_path), check=False)

                    cmds.append("cd ~/deploy/{tid} && docker compose up -d || docker-compose up -d".format(tid=req.task_id))
                    
                    for c in cmds:
                        plan_obj.commands.append(" ".join(self._ssh_cmd(s_schema, c)))
                    if req.execute:
                        for c in cmds:
                            subprocess.run(self._ssh_cmd(s_schema, c), check=False)
            
            elif req.environment == "k8s":
                mfs = self.generate_k8s(req)
                idx = 0
                for text in mfs:
                    fp = self._artifact_path(req.task_id, f"{req.task_id}_{idx}.yaml")
                    with open(fp,"w",encoding="utf-8") as f:
                        f.write(text)
                    plan_obj.artifacts.append(fp)
                    idx += 1
                for server in servers:
                    ...
                    for fp in plan_obj.artifacts:
                        remote = f"/root/deploy/{os.path.basename(fp)}"
                        plan_obj.commands.append(" ".join(self._scp_cmd(s_schema, fp, remote)))
                        plan_obj.commands.append(" ".join(self._ssh_cmd(s_schema, f"kubectl apply -f {remote}")))
                        if req.execute:
                            subprocess.run(self._scp_cmd(s_schema, fp, remote), check=False)
                            subprocess.run(self._ssh_cmd(s_schema, f"kubectl apply -f {remote}"), check=False)
            
            elif req.environment == "helm":
                values_fp = self._artifact_path(req.task_id, "values.yaml")
                lines = []
                lines.append("global:")
                ...
                with open(values_fp, "w", encoding="utf-8") as f:
                    f.write("\\n".join(lines))
                plan_obj.artifacts.append(values_fp)
                for server in servers:
                    s_schema = ServerConfigSchema(
                        id=server.id, name=server.name, host=server.host, port=server.port,
                        user=server.user, auth_method=server.auth_method,
                    ...
                    plan_obj.commands.append(" ".join(self._scp_cmd(s_schema, values_fp, remote_values)))
                    ...
                    plan_obj.commands.append(" ".join(self._ssh_cmd(s_schema, helm_cmd)))
                    if req.execute:
                        subprocess.run(self._scp_cmd(s_schema, values_fp, remote_values), check=False)
                        subprocess.run(self._ssh_cmd(s_schema, helm_cmd), check=False)
            else:
                fp = self._artifact_path(req.task_id, "install.sh")
                lines = []
                lines.append("set -e")
                ...
                for server in servers:
                    ...
                    plan_obj.commands.append(" ".join(self._scp_cmd(s_schema, fp, remote)))
                    plan_obj.commands.append(" ".join(self._ssh_cmd(s_schema, f"bash {remote}")))
                    if req.execute:
                        subprocess.run(self._scp_cmd(s_schema, fp, remote), check=False)
                        subprocess.run(self._ssh_cmd(s_schema, f"bash {remote}"), check=False)
            
            plan_obj.status = "completed" if req.execute else "pending"
        except Exception as e:
            plan_obj.status = "error"
            plan_obj.error = str(e)
        
        plan_obj.save()
        return plan_obj

    def execute_async(self, pid: str):
        try:
            plan = DeployPlan.objects.get(id=pid)
        except DeployPlan.DoesNotExist:
            return

        def _runner():
            plan.status = "running"
            total = max(1, len(plan.commands))
            plan.save()
            
            for i, cmd in enumerate(plan.commands):
                try:
                    plan.logs.append(f"$ {cmd}")
                    # Note: Need to be careful with concurrency updating JSONField in list.
                    # Ideally append via atomic op or just overwrite. Overwriting is fine for single thread.
                    # But we should refresh from DB? No, this thread owns the write.
                    # Actually, if we want real-time logs in frontend, we should save frequently.
                    
                    proc = subprocess.run(cmd, shell=True, capture_output=True, text=True)
                    if proc.stdout:
                        plan.logs.extend(proc.stdout.strip().splitlines())
                    if proc.stderr:
                        plan.logs.extend(proc.stderr.strip().splitlines())
                    
                    if proc.returncode != 0:
                        plan.status = "error"
                        plan.error = f"command failed: {cmd}"
                        plan.progress = int((i+1) * 100 / total)
                        plan.save()
                        return
                except Exception as e:
                    plan.status = "error"
